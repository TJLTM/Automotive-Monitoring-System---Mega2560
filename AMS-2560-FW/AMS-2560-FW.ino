...
void PressureSensors() {
  ...
  long OilSum = 0;
  for (int x = 0; x < Samples; x++) {
    FuelSum = FuelSum + analogRead(FuelPressureSensorsPinAtCarb);
    FuelSum1 = FuelSum1 + analogRead(FuelPressureSensorsPinBeforeCooler);
    OilSum = OilSum + analogRead(OilPressureSensorsPin);
    
  }

  float FuelPSIatCarb = 1.2 * 7.5 * ConversionFactor * (FuelSum / Samples);
  float FuelPSIatPump = 1.2 * 7.5 * ConversionFactor * (FuelSum1 / Samples);
  float OilPSI = 25 * ConversionFactor * (OilSum / Samples) - 12.5;
  FuelPressureWarning(FuelPSIatCarb);

  
  float FuelPressure_carb = 0.0;
  float FuelPressure_pump = 0.0;
  float OilPressure = 0.0;
  String Units = "PSI";
  if (Units == "M") {
    FuelPressure_carb = ConvertPSItoKPa(FuelPSIatCarb);
    FuelPressure_pump = ConvertPSItoKPa(FuelPSIatPump);
    OilPressure = ConvertPSItoKPa(OilPSI);
    Units = "KPa";
  }
  else {
    FuelPressure_carb = FuelPSIatCarb;
    FuelPressure_pump = FuelPSIatPump;
    OilPressure = OilPSI;
  }

  LastFuelPatCarb = FuelPressure_carb;
  LastFuelPatPump = FuelPressure_pump;
  String LogEntry = "Units," + Units + ",Fuel At Carb," + String(FuelPressure_carb) + ",Fuel At pump," + String(FuelPressure_pump) + ",Oil," + String(OilPressure);
  AddLogEntry(0, "Pressure Sensors", LogEntry);
}
...
void GetTemps(int WhichPort) {
  SendItOut("%R,Temps,Units," + "" + ",Water," + String(LastWaterTemp) + ",," + String(LastCarbTemp) + ",," + String(LastAmbientTemp) , WhichPort);
}
//------------------------------------------------------------------
//Alarm and Warnings
//------------------------------------------------------------------
